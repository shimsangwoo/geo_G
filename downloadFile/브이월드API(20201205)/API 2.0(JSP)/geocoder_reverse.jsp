...
<html>
<head>
<title>(예제)지오코더API 2.0(주소조회)</title>
...
div.maintitle {width:95%;height:50px;font-size:18px;line-height:50px;background-color:green;color:white;font-weight:bold;padding-left:20px;}
</style>
...
<body>
	<div id="desc" style="padding:0px 0px;">
		<div id="toolbar">	
			<div class="maintitle">(예제) 지오코더 API 2.0 (주소조회)</div>
			<div class="temporary">
			...
			* 입력한 경위도 좌표값들이 요청좌표계(crs)에 맞는 좌표로 변환됩니다.
			...
			<table class="board-write txt-left">
				...
				<tbody>
					...
					<tr><th>오퍼레이션(request)</th><td>M/1</td><td><input type="text" id="request" value="getaddress" readonly/>
					...
					<tr><th>검색좌표(point)</th><td>M/1</td><td><input type="text" id="point" value="126.978275264,37.566642192"  readonly/></td></tr>
					<tr><th>요청좌표계(crs)</th><td>O/1</td><td><input type="text" id="crs" value="epsg:4326" readonly/>
					...
					<tr><th>우편번호여부(zipcode)</th><td>O/1</td><td><input type="text" id="zipcode" value="true" readonly/>
						<select id="zipcode_list" onchange="changeValue(this.id, this.value);">
