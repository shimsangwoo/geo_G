		...
		<div id="toolbar">	
			...
			<table class="board-write txt-left">
				...
				<tbody>
					...
					<tr><th>인증키(key)</th><td>M/1</td><td><input type="text" id="key" value="<%=request.getParameter("key")==null?"483E0418-2F46-3223-80A1-F66D16A24685":request.getParameter("key")%>" /></td></tr>
					<tr><th>결과포맷(format)</th><td>O/1</td><td><input type="text" id="format" value="<%=request.getParameter("format")==null?"xml":request.getParameter("format")%>" readonly/>
						<select id="format_list" onchange="changeValue(this.id, this.value);">
						<option value="xml">xml</option>
						<option value="json">json</option>
						</select>
					</td></tr>
					<tr><th>오류결과포맷(errorFormat)</th><td>O/1</td><td><input type="text" id="errorformat" value="<%=request.getParameter("errorformat")==null?"xml":request.getParameter("errorformat")%>" readonly/>
						<select id="errorformat_list" onchange="changeValue(this.id, this.value);">
						<option value="xml">xml</option>
						<option value="json">json</option>
						</select>
					</td></tr>
					<tr><th>결과건수(size)</th><td>O/1</td><td><input type="text" id="size" value="<%=request.getParameter("size")==null?"10":request.getParameter("size")%>"/></td></tr>
					<tr><th>페이지번호(page)</th><td>O/1</td><td><input type="text" id="page" value="<%=request.getParameter("page")==null?"1":request.getParameter("page")%>"/></td></tr>
					<tr><th>조회할 데이터(data)</th><td>M/1</td><td><input type="text" id="data" value="<%=request.getParameter("data")==null?"LT_C_UQ111":request.getParameter("data")%>"/> 예) LP_PA_CBND_BUBUN(지적도)</td></tr>
					<tr><th>지오메트리 필터(geomFilter)</th><td>O/1</td><td><input type="text" id="geomFilter" value="<%=request.getParameter("geomFilter")==null?"LINESTRING(14135552.266976 4518634.8999433,14134863.139393 4518195.3870307)":request.getParameter("geomFilter")%>" style="width:90%"/></td></tr>
					<tr><th>확장 버퍼(buffer)</th><td>O/1</td><td><input type="text" id="buffer" value="<%=request.getParameter("buffer")==null?"":request.getParameter("buffer")%>"/> 단위 : m</td></tr>
					<tr><th>속성 필터(attrFilter)</th><td>O/n</td><td><input type="text" id="attrFilter" value="<%=request.getParameter("attrFilter")==null?"uname:like:일반상업지역":request.getParameter("attrFilter")%>"/> 지적도일 경우 예) pnu:like:11140</td></tr>
					<tr><th>결과컬럼(columns)</th><td>O/1</td><td><input type="text" id="columns" value="<%=request.getParameter("columns")==null?"":request.getParameter("columns")%>"/></td></tr>
					<tr><th>지오메트리 반환여부(geometry)</th><td>O/1</td><td><input type="text" id="geometry" value="<%=request.getParameter("geometry")==null?"":request.getParameter("geometry")%>" readonly/>
					...
					<tr><th>속성 반환여부(attribute)</th><td>O/1</td><td><input type="text" id="attribute" value="<%=request.getParameter("attribute")==null?"":request.getParameter("attribute")%>" readonly/>
					...
					<tr><th>좌표계(crs)</th><td>O/1</td><td><input type="text" id="crs" value="<%=request.getParameter("crs")==null?"epsg:3857":request.getParameter("crs")%>" readonly/>
